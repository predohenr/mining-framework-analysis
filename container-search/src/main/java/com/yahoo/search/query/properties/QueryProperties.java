...
public class QueryProperties extends Properties {
    ...
    private static Map<CompoundName, GetterSetter> createPropertySetterMap() {
        ...
        map.put(CompoundName.fromComponents(Ranking.RANKING, Ranking.RERANKCOUNT), GetterSetter.of(query -> query.getRanking().getSecondPhase().getRerankCount(), (query, value) -> query.getRanking().getSecondPhase().setRerankCount(asInteger(value, null)))); // TODO: Remove on Vespa 9
        ...
        map.put(CompoundName.fromComponents(Ranking.RANKING, SecondPhase.SECOND_PHASE, Ranking.RERANKCOUNT), GetterSetter.of(query -> query.getRanking().getSecondPhase().getRerankCount(), (query, value) -> query.getRanking().getSecondPhase().setRerankCount(asInteger(value, null))));
        ...
        map.put(CompoundName.fromComponents(Ranking.RANKING, GlobalPhase.GLOBAL_PHASE, Ranking.RERANKCOUNT), GetterSetter.of(query -> query.getRanking().getGlobalPhase().getRerankCount(), (query, value) -> query.getRanking().getGlobalPhase().setRerankCount(asInteger(value, null))));
        map.put(CompoundName.fromComponents(Ranking.RANKING, GlobalPhase.GLOBAL_PHASE, Ranking.RANKSCOREDROPLIMIT), GetterSetter.of(query -> query.getRanking().getGlobalPhase().getRankScoreDropLimit(), (query, value) -> query.getRanking().getGlobalPhase().setRankScoreDropLimit(asDouble(value, null))));
